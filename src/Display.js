...
import {netDate, timestampToString} from './utils';
...
export default class Display extends Component {
    render() {
        ...
        if(this.props.screen){
            display = [
                <p key="heading" className="heading"><span>{this.props.sport.N}</span> <span>LIVE</span></p>,
                <small key="test">testing event: {this.props.eventId}</small>,
                <h3 key="event" className="event">
                    <span className="start">{netDate(this.props.data[0].D)}</span>
                    <span className="name">{this.props.data[0].N}</span>
                    <span>{timestampToString(this.props.data[0].TS)}</span>
                    <span className="score-details">({this.props.data[0].SS})</span>
                    <span className="totalScore">{this.props.data[0].HS+':'+this.props.data[0].AS}</span>
